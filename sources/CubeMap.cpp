...
HRESULT CubeMap::LoadCubeMap(WCHAR* directory, WCHAR* name, WCHAR* extension){
  ...
  SetName(name);
  WCHAR cubemapfile[120];
  WCHAR cubemappathrel[120];WCHAR cubemappath[120];
  Concat( cubemapfile, name, extension );
  Concat( cubemappathrel, directory, cubemapfile );
  AppendToRootDir(cubemappath, cubemappathrel);
  
  PD(L"Path: ", cubemappath);

  hr = D3DXCreateCubeTextureFromFile( mDevice, cubemappath, &mCubeTexture );
  ...
  WCHAR msg[120];
  Concat(msg, L"load effect file ", effectName);
  PD(hr, msg);
  if(FAILED(hr)) return hr;
